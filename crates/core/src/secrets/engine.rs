...
///
/// The byte-counting loop is manually unrolled into 4-byte chunks via
/// [`<[u8]>::chunks_exact`] so the inner increments can run with
/// instruction-level parallelism on most CPUs. The remainder (0..=3
/// bytes) is processed via the trailing tight loop.
pub(crate) fn shannon_entropy(s: &str) -> f64 {
    ...
    for chunk in chunks.by_ref() {
        // Four independent loads / increments — no inter-iteration
        // dependency, leaving the OOO scheduler free to pipeline them.
...
/// Run [`scan_and_redact`] across `inputs` in parallel via rayon.
///
/// Returns a `Vec<ScanResult>` of the same length and order as `inputs`.
/// The compiled `RuleSet` is shared by reference across worker threads
/// — the underlying `Regex` and `AhoCorasick` types are thread-safe and
/// the keyword index is wired through a process-wide `OnceLock` for the
/// vendored ruleset, so no per-thread setup cost is incurred beyond the
/// scan itself.
pub fn scan_and_redact_batch(inputs: &[&str], ruleset: &RuleSet) -> Vec<ScanResult> {
    inputs
        .par_iter()
        .map(|content| scan_and_redact(content, ruleset))
        .collect()
}
...
"#;
    ...
    // -----------------------------------------------------------------
    // scan_and_redact_batch parity
    // -----------------------------------------------------------------

    #[test]
    fn batch_matches_sequential_scan_for_single_input() {
        let inputs = ["token = AKIAIOSFODNN7EXAMPLE\n"];
        let batch = scan_and_redact_batch(&inputs, ruleset::vendored());
        let seq = scan_and_redact(inputs[0], ruleset::vendored());
        assert_eq!(batch.len(), 1);
        assert_eq!(batch[0].redacted_content, seq.redacted_content);
        assert_eq!(batch[0].redactions.len(), seq.redactions.len());
        for (a, b) in batch[0].redactions.iter().zip(seq.redactions.iter()) {
            assert_eq!(a.rule_id, b.rule_id);
            assert_eq!(a.line, b.line);
            assert_eq!(a.byte_offset, b.byte_offset);
            assert_eq!(a.matched_excerpt, b.matched_excerpt);
        }
    }

    #[test]
    fn batch_preserves_order_across_mixed_inputs() {
        let inputs = [
            "no secrets here\n",
            "token = AKIAIOSFODNN7EXAMPLE\n",
            "no secrets here either\n",
            "x = ghp_1234567890abcdefghijklmnopqrstuvwxyz\n",
        ];
        let batch = scan_and_redact_batch(&inputs, ruleset::vendored());
        assert_eq!(batch.len(), inputs.len());
        for (i, content) in inputs.iter().enumerate() {
            let seq = scan_and_redact(content, ruleset::vendored());
            assert_eq!(
                batch[i].redacted_content, seq.redacted_content,
    ...
    fn batch_empty_input_returns_empty_vec() {
        let batch = scan_and_redact_batch(&[], ruleset::vendored());
