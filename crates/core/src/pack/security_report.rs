...
/// Estimated bytes of output a single redaction tag takes (XML form is
/// ~80 bytes for a typical row; the markdown table row is ~70; the
/// plain-text line is ~60). 200 covers the worst-case wire format and
/// leaves headroom for unusually long rule ids / paths.
const ESTIMATED_BYTES_PER_REDACTION: usize = 200;
/// Bytes of fixed-overhead content in every non-empty security report —
/// the opening / closing tags or the markdown header. 64 is enough for
/// every emitter variant in this module.
const SECURITY_REPORT_FIXED_OVERHEAD: usize = 64;

fn estimated_capacity(num_redactions: usize) -> usize {
    ESTIMATED_BYTES_PER_REDACTION
        .saturating_mul(num_redactions)
        .saturating_add(SECURITY_REPORT_FIXED_OVERHEAD)
}
...
pub fn emit_xml(redactions: &[PackRedaction]) -> String {
    ...
    let mut out = String::with_capacity(estimated_capacity(redactions.len()));
    let _ = writeln!(
        out,
