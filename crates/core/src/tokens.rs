...
pub fn count_all(text: &str) -> CoreResult<TokensPerModel> {
    // Delegate to the parallel implementation so the public API picks up
    // the speedup automatically. The parallel path produces identical
    // counts to the sequential one (rayon::join is deterministic and
    // each branch is pure) — see the parity tests below.
    count_all_parallel(text)
}

/// Compute every per-model token count for `text`, splitting the work
/// across two rayon-parallel branches:
///
/// 1. **tiktoken group** — `Gpt4o`, `Claude`, `GeminiApprox`. These
///    share two underlying BPE encoders (`o200k_base`,
///    `cl100k_base`), and `GeminiApprox` is a constant-time scaling
///    of the `Gpt4o` count, so this branch is dominated by the two
///    real encodes.
/// 2. **HuggingFace group** — `Llama3`, `Qwen2_5`, `DeepSeek`,
///    `Mistral`. Each lazily parses its vendored `tokenizer.json`
///    once; afterwards the per-call cost is the encode itself.
///
/// `rayon::join` runs the two branches concurrently when a worker is
/// available and falls through to sequential execution otherwise; this
...
/// Compute `(gpt4o, claude, gemini_approx)` in one short call sequence.
/// Hot inner loop of [`count_all_parallel`] — extracted so the rayon
/// closure stays tiny.
...
/// Compute `(llama3, qwen2_5, deep_seek, mistral)` in one call. Hot
/// inner loop of the HF branch in [`count_all_parallel`].
fn count_hf_group(text: &str) -> CoreResult<(u32, u32, u32, u32)> {
    let llama3 = count(text, TokenModel::Llama3)?;
    let qwen2_5 = count(text, TokenModel::Qwen2_5)?;
...
mod tests {
    ...
    // --- count_all_parallel + count_batch (parity) -------------------------

    /// Sequential helper used as the parity oracle: hits each model in turn
    /// without going through [`count_all_parallel`]. Mirrors what
    /// `count_all` did before the parallel rewrite.
    fn count_all_sequential_oracle(text: &str) -> CoreResult<TokensPerModel> {
        Ok(TokensPerModel {
    ...
    #[test]
    fn parallel_matches_sequential_for_simple_text() {
    ...
    #[test]
    fn parallel_matches_sequential_for_unicode_text() {
        let input = "日本語 — Café résumé naïve façade. The quick fox 12 dogs.";
        let par = count_all_parallel(input).unwrap();
        let seq = count_all_sequential_oracle(input).unwrap();
        assert_eq!(par, seq);
    }
    ...
    #[test]
    fn parallel_empty_input_zero_for_all_seven_models() {
        let par = count_all_parallel("").unwrap();
        assert_eq!(par, TokensPerModel::default());
    }

    #[test]
    fn count_all_delegates_to_parallel_path() {
        // After the rewrite, the public `count_all` must agree with the
        // explicit parallel path on every model.
    ...
    #[test]
    fn parallel_parity_per_model_for_long_corpus() {
        // Deliberately long input so every model does real work and any
        // race/state bug in the parallel path would surface as a count
        // mismatch.
        let input = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, \
                     sed do eiusmod tempor incididunt ut labore et dolore magna \
                     aliqua. The quick brown fox jumps over 12 lazy dogs — and 3.14 \
                     pies, costing $42.99 each. Café résumé naïve façade. \
